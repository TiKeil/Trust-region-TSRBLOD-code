...
def enrichment_step(mu, reductor, U=None, P=None, pool=None, global_solves=False):
    ...
    # out_1 and out_2 are u and p
    out_1, out_2 = new_reductor.extend_bases(mu, U=U, P=P, pool=pool, global_solves=global_solves)
...
def Adaptive_Relaxed_TR_algorithm(opt_rom, reductor, parameter_space, TR_parameters=None, extension_params=None, opt_fom=None,
    ...
    while k < TR_parameters['max_iterations']:
        ...
        if not point_rejected:
            if model_has_been_enriched:
                # using the opt_rom first
                gradient = opt_rom.output_functional_hat_gradient(mu_k, U=u, P=p)
                mu_box = opt_rom.primal_model.parameters.parse(mu_k.to_numpy() - gradient)
                first_order_criticity = mu_k.to_numpy() - projection_onto_range(parameter_space, mu_box).to_numpy()
                normgrad = np.linalg.norm(first_order_criticity)

                if normgrad <= TR_parameters['FOC_tolerance']:
                    print('\nStopping criterion on reduced level fulfilled: reduced FOC condition: {}'.format(normgrad))
                    print('Computing FOM FOC criterion ...')
                    ...
                    if normgrad > TR_parameters['FOC_tolerance']:
                        print('FOM FOC not fulfilled, yet. Enriching with computed snapshots')
                        opt_rom, reductor, out_1, out_2 = enrichment_step(mu_k, reductor, pool=pool,
                                                                          global_solves=True)
